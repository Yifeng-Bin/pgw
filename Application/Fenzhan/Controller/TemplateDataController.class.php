...
class TemplateDataController extends CommonController {
    ...
    public function dataList() {
        ...
        $position_type=D('TemplateDataPosition')->getTemplateDataPosition($position_id);
        ...
        if($position_type=='worker'||$position_type=='link' ){
            $where['t.region_id'] = ADMIN_CITY_ID;
        }
        $page = new \Com\Logdd\Page(D('TemplateData')->alias('t')->where($where)->count(), 50);
        $this->pageInfo = $page->getPageInfo();
        $this->position_id = $position_id;
        $this->positionInfo = D('TemplateDataPosition')->getTemplateDataPosition($position_id);
        $this->list = D('TemplateData')       
        ->field('t.data_id,t.region_id,t.data_name,t.data_text,t.data_link,t.data_image,t.start_time,t.end_time,t.click_count,t.status,t.sort,r.region_name')
        ->alias('t')->join('left join __REGION__ as r on r.region_id = t.region_id')
        ->where($where)->select();
